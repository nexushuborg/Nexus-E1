...
const CustomCalendar = ({
  ...
  const isSolved = (day) => solvedDays.some((solvedDay) => isSameDay(solvedDay, day));
  ...
  const DayCell = ({ day }) => {
    ...
    // Base cell classes
    let cellClasses =
      "h-8 w-8 flex items-center justify-center text-sm rounded-full transition-colors duration-200";
    ...
    // Disable default hover/focus from Button component
    const noHoverFocus = "hover:bg-transparent focus:bg-transparent focus:ring-0";
    ...
    return (
      ...
      >
        {solved ? <CheckCircle2 className="h-5 w-5 text-purple-600" /> : day.getDate()}
      </Button>
    );
  };
      ...
      <div className="absolute top-4 right-4 z-10">
        <div className="relative w-16 h-20">
          ...
          <div className="relative z-10 mb-4 flex flex-col items-center justify-center h-full text-foreground">
            ...
            <div className="text-xs font-medium uppercase">{format(new Date(), "MMM")}</div>
          </div>
        </div>
      </div>
