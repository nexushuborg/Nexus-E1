...
const CustomCalendar = ({
  ...
  const isSolved = (day) => {
    return solvedDays.some((solvedDay) => isSameDay(solvedDay, day));
  };
  const DayCell = ({ day }) => {
    if (!day) {
      return <div className="h-8 w-8 text-center text-sm p-0"></div>;
    }
    const today = isToday(day);
    const solved = isSolved(day);
    const isSelected = selectedDate && isSameDay(selectedDate, day);
    let cellClasses =
      "h-8 w-8 text-center text-sm p-0 relative rounded-full flex items-center justify-center";
    if (isSelected) {
      cellClasses +=
        " ring-1 ring-primary-foreground bg-primary text-primary-foreground";
    } else if (solved) {
    ...
    <div className="w-full">
      ...
      <div className="flex justify-around mt-4 mb-2">
        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((day) => (
          <div
            key={day}
            className="text-muted-foreground rounded-md w-8 font-normal text-xs text-center"
          >
            {day}
          </div>
        ))}
...
export function ActivityCalendar() {
  ...
  return (
    <Card className="rounded-2xl h-full relative overflow-hidden p-2 shadow-lgdark:bg-red-800 card">
